...
namespace NEAT {
	...
	public:
		...
		int cur_node_id = 0;  //Current label number available
		double cur_innov_num = 0;

		int last_species = 0;  //The highest species number
		...
		int winnergen = 0; //An integer that when above zero tells when the first winner appeared

		// ******* When do we need to delta code? *******
		double highest_fitness = 0;  //Stagnation detector
		int highest_last_changed = 0; //If too high, leads to delta coding

		// Separate the Organisms into species
		bool speciate();

		// Print Population to a file specified by a string 
		bool print_to_file(std::string outFile, bool reloadBefore = false, bool removeWorst = false);
		bool print_to_mmap(std::string filename, bool reloadBefore = false, bool removeWorst = false);

		//reload population to get newly evaluated individuals
		bool reload(const char* file, bool lock, bool mmap = false);
		...
		Population(const char *filename, bool mmap = false);
