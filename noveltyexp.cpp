...
Population *maze_fitness_realtime(char* outputdir, const char *mazefile, int par) {
	...
	//starter gene file
	ifstream iFile("../../input_files/mazestartgenes", ios::in);
...
Population *maze_novelty_realtime(char* outputdir, const char* mazefile, int par) {
	...
	string genefile = "../../input_files/mazestartgenes";
	cout << "(startgenes) " << genefile << endl;
	//starter genes file
	ifstream iFile(genefile, ios::in);
...
double mazesimStep(Environment* newenv, Network *net, vector< vector<float> > &dc)
{
	...
	if (dist <= 1) dist = 1;
	double fitness = 5.0 / dist; //used for accumulated fitness (obsolete)
