...
Population::Population(const char *filename, bool mmap) {
...
//        }
    ...
    while (!(*input).eof())
    {
        ...
        if (strcmp(curword, "genomestart") == 0)
        {
            ...
            new_genome = new Genome(idcheck, (*input));
        ...
        else if (strcmp(curword, "organism_params") == 0) {
            organisms[organisms.size() - 1]->readParams(ss);
        }
        else if (strcmp(curword, "noveltyitemstart") == 0) {
            organisms[organisms.size() - 1]->noveltypoint = new noveltyitem((*input));
            organisms[organisms.size() - 1]->noveltypoint->genotype = new Genome(*organisms[organisms.size() - 1]->gnome);
            organisms[organisms.size() - 1]->noveltypoint->phenotype = new Network(*organisms[organisms.size() - 1]->net);
        }
        else if (strcmp(curword, "species") == 0) {
            species.push_back(new Species((*input)));
        }
        else if (strcmp(curword, "innovation") == 0) {
            char argline[1024];
            ss.getline(argline, 1024);
            innovations.push_back(new Innovation(argline));
        }
        else if (strcmp(curword, "params") == 0) {
            ss >> cur_node_id >> cur_innov_num >> last_species >> mean_fitness >> variance
                >> standard_deviation >> winnergen >> highest_fitness >> highest_last_changed >> NEAT::compat_threshold;
        }
    ...
    //link organism to species and vice-versa
    for (vector<Species*>::iterator s = species.begin(); s != species.end(); ++s) {
        vector<int> ids = (*s)->organisms_id;
        for (vector<int>::iterator id = ids.begin(); id != ids.end(); ++id) {
            for (vector<Organism *>::iterator org = organisms.begin(); org != organisms.end(); ++org) {
                if ((*org)->gnome->genome_id == *id) {
                    (*s)->add_Organism(*org);
                    (*org)->species = *s;
                }
            }
        }
        if (!ids.empty() && (*s)->organisms.empty()) {
            cout << "Error : could not assign id to organism !";
        }
    }

    //auto assign species if misread
    if(species.size() > 0) {
        for (vector<Organism *>::iterator org = organisms.begin(); org != organisms.end(); ++org) {
            if (!(*org)->species) {
                (*org)->species = (*species.begin());
                // ? (*species.begin())->add_Organism((*org));
            }
        }
    }
    //speciate();
...
bool Population::print_to_file_by_species(string filename) {
...
bool Population::reload(const char *filename, bool lock, bool mmap) {
        ...
        while (!(*input).eof())
        {
            ...
            if (strcmp(curword, "genomestart") == 0)
            {
                ...
                for (auto org = organisms.begin(); org != organisms.end(); ++org) {
					if ((*org)->gnome->genome_id == idcheck && (*org)->noveltypoint) {
						//skip organism, it is already loaded with novelty data
						cid = idcheck;
						found = true;
						break;
					}
					else if((*org)->gnome->genome_id == idcheck){
						found = true;
                        break;
					}
					i++;
				...
				orgid = i;
				if(found) continue;  //do not duplicate organism
                ...
                new_genome = new Genome(idcheck, (*input));
                Organism *o = new Organism(0, new_genome, 1, metadata);
                ...
                (*species.begin())->add_Organism(o);
                (o)->species = (*species.begin());
            }
            else if(cid == idcheck) continue;	//skip if we already know organism
			else if (strcmp(curword, "organism_params") == 0) {
				organisms[orgid]->readParams(ss);
			}
            else if (strcmp(curword, "noveltyitemstart") == 0) {
                organisms[orgid]->noveltypoint = new noveltyitem((*input));
                organisms[orgid]->noveltypoint->genotype = new Genome(*organisms[organisms.size() - 1]->gnome);
                organisms[orgid]->noveltypoint->phenotype = new Network(*organisms[organisms.size() - 1]->net);
            }
//            else if (strcmp(curword, "species") == 0) {
//                species.push_back(new Species((*input)));
//            }
...
//            else if (strcmp(curword, "params") == 0) {
//                ss >> cur_node_id >> cur_innov_num >> last_species >> mean_fitness >> variance
//                   >> standard_deviation >> winnergen >> highest_fitness >> highest_last_changed;
//            }
            else if (strcmp(curword, "/*") == 0)
            {
...
bool Population::print_to_file(string filename, bool reloadBefore, bool removeWorst) {
	std::vector<Organism*>::iterator curorg;
	std::vector<Species*>::iterator spec;
	std::vector<Innovation*>::iterator ino;

#ifndef _WIN32
	lockFile();
#endif
	if(reloadBefore) reload(filename.c_str(), false);

	//Remove the worst organism
	while(removeWorst && organisms.size() > NEAT::pop_size) remove_worst();
	...
	//print all simple variables
	outFile << "params " << cur_node_id << " " << cur_innov_num << " " << last_species << " " << mean_fitness << " " << variance
		<< " " << standard_deviation << " " << winnergen << " " << highest_fitness << " " << highest_last_changed << " " << NEAT::compat_threshold << endl;

	for (spec = species.begin(); spec != species.end(); ++spec) {
		(*spec)->serialize(outFile);
	}

	for (ino = innovations.begin(); ino != innovations.end(); ++ino) {
		(*ino)->serialize(outFile);
	}

	//Print all the Organisms' Genomes to the outFile
	for (curorg = organisms.begin(); curorg != organisms.end(); ++curorg) {
		((*curorg)->gnome)->print_to_file(outFile);
		(*curorg)->serializeParams(outFile);
...
//save population in memory map for faster access
bool Population::print_to_mmap(string filename, bool reloadBefore, bool removeWorst) {
	std::vector<Organism*>::iterator curorg;
	std::vector<Species*>::iterator spec;
	std::vector<Innovation*>::iterator ino;

    if(reloadBefore) reload(filename.c_str(), false, true);
    ...
    //print all simple variables
    str << "params " << cur_node_id << " " << cur_innov_num << " " << last_species << " " << mean_fitness << " " << variance
            << " " << standard_deviation << " " << winnergen << " " << highest_fitness << " " << highest_last_changed << " " << NEAT::compat_threshold << endl;
    ...
    //Print all the Organisms' Genomes to the str
    for (curorg = organisms.begin(); curorg != organisms.end(); ++curorg) {
        ((*curorg)->gnome)->print_to_file(str);
        (*curorg)->serializeParams(str);
        if((*curorg)->noveltypoint)
            ((*curorg)->noveltypoint)->SerializeNoveltyPoint(str);
    }
