...
//Perform evolution on single pole balancing, for gens generations
Population *pole1_test(int gens) {
	...
	int expcount;
	int status;
	int *runs = new int[NEAT::num_runs];
	int totalevals;
...
int go_cart(Network *net, int max_steps, int thresh)
{
	...
	if (random_start) {
		...
		x = (randfloat() * 4800) / 1000.0 - 2.4;
		x_dot = (randfloat() * 2000) / 1000.0 - 1;
		theta = (randfloat() * 400) / 1000.0 - .2;
		theta_dot = (randfloat() * 3000) / 1000.0 - 1.5;
	}
	else
...
Population *pole2_test(int gens, int velocity) {
	...
	int highscore;
	int **record = new int*[NEAT::num_runs];
	for (int i = 0; i < NEAT::num_runs; ++i)
		record[i] = new int[1000];
	double recordave[1000];
	int **genesrec = new int*[NEAT::num_runs];
	for (int i = 0; i < NEAT::num_runs; ++i)
		genesrec[i] = new int[1000];
	double genesave[1000];
	int **nodesrec = new int*[NEAT::num_runs];
	for (int i = 0; i < NEAT::num_runs; ++i)
		nodesrec[i] = new int[1000];
	double nodesave[1000];
	int *winnergens = new int[NEAT::num_runs];
	int initcount;
