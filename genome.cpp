...
Genome::Genome(int id, std::istream &iFile) {
	...
	while (!done && !iFile.eof()) {
		...
		if (strcmp(curword,"genomeend")==0) {
			...
			//iFile>>idcheck;
			if (idcheck!=genome_id) printf("ERROR: id mismatch in genome ! %d and %d\n",idcheck, genome_id);
			done=1;
		}
		...
		//Ignore genomestart if it hasn't been gobbled yet
            ...
            ss >> parent1 >> parent2 >> struct_change;
...
void Genome::print_to_file(std::ofstream &outFile) {
  ...
  outFile<<"genomestart "<<genome_id<<std::endl;
    ...
    outFile << "params "<< parent1 << " " << parent2 << " " << struct_change << std::endl;
			...
			while ((thegene!=genes.end()) &&
				...
				if (thegene!=genes.end())
					...
					if (((nodep1->gen_node_label)==OUTPUT)||
						((nodep2->gen_node_label)==OUTPUT))     //edited by bat : (node->type)==OUTPUT
						recurflag=true;
...
void Genome::randomize_traits() {
	...
	for(curnode=nodes.begin();curnode!=nodes.end();++curnode) {
		traitnum=randint(1,numtraits - 2); //randomize trait
		(*curnode)->trait_id=traitnum;

		curtrait=traits.begin();
		while( curtrait!=traits.end() && ((*curtrait)->trait_id)!=traitnum)
			++curtrait;
