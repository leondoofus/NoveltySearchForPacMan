...
bool Organism::serializeParams(std::ostream &outFile){
    outFile << "organism_params " << fitness << " " << error << " " << winner  << " " << expected_offspring << " " <<
    	generation << " " << eliminate << " " << champion << " " << super_champ_offspring << " " << pop_champ << " " <<
    	pop_champ_child  << " " <<  high_fit << " " << time_alive << " " << mut_struct_baby << " " << mate_baby << " " <<
    	modified << std::endl;
    return true;
}

bool Organism::readParams(std::stringstream &iFile){
	iFile >> fitness >> error >> winner  >> expected_offspring >> generation >> eliminate >> champion >>
	super_champ_offspring >> pop_champ >> pop_champ_child  >>  high_fit >> time_alive >> mut_struct_baby >>
	mate_baby >> modified;
	return true;
}
